...
assert.match(sitemap, /https:\/\/clickbaitpaysus\.com\/profit-calculator\.html/, "sitemap includes calculator page");
assert.equal((sitemap.match(/profit-calculator\.html/g) || []).length, 1, "sitemap contains one calculator entry");
...
console.log(`Site integration tests passed: ${htmlFiles.length} HTML pages, ${dashboardFiles.length} dashboard menus, matching campaign tables, valid local paths, JSON-LD, CTA placement, and calculator metadata.`);
